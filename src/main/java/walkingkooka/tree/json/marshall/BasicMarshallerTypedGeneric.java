...
/**
 * A {@link BasicMarshaller} that handles non built in mappings, using two
 * {@link BiFunction} one for reading from a {@link JsonNode} and the other to do the inverse.
 */
final class BasicMarshallerTypedGeneric<T> extends BasicMarshallerTyped<T> {

    static <T> BasicMarshallerTypedGeneric<T> with(final String typeName,
        ...
        return new BasicMarshallerTypedGeneric<>(typeName, from, to, type, all);
    }

    private BasicMarshallerTypedGeneric(final String typeName,
                                        final BiFunction<JsonNode, FromJsonNodeContext, T> from,
                                        final BiFunction<T, ToJsonNodeContext, JsonNode> to,
                                        final Class<T> type,
    ...
    /**
     * Registers this {@link BasicMarshallerTypedGeneric} returning a {@link Runnable} which can be used to remove this mapping.
     */
    synchronized Runnable registerGeneric() {
        this.registerTypeNameAndType();
        this.registerTypes(this.types);

        return BasicMarshallerTypedGenericRunnable.with(this);
    }

    /**
     * Eventually called by {@link BasicMarshallerTypedGenericRunnable#run()}
     */
    void remove() {
