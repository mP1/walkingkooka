...
/**
 * A {@link BasicMapper} that handles non built in mappings, using two
 * {@link BiFunction} one for reading from a {@link JsonNode} and the other to do the inverse.
 */
final class BasicMapperTypedGeneric<T> extends BasicMapperTyped<T> {

    static <T> BasicMapperTypedGeneric<T> with(final String typeName,
        ...
        return new BasicMapperTypedGeneric<>(typeName, from, to, type, all);
    }

    private BasicMapperTypedGeneric(final String typeName,
                                    final BiFunction<JsonNode, FromJsonNodeContext, T> from,
                                    final BiFunction<T, ToJsonNodeContext, JsonNode> to,
                                    final Class<T> type,
    ...
    /**
     * Registers this {@link BasicMapperTypedGeneric} returning a {@link Runnable} which can be used to remove this mapping.
     */
    synchronized Runnable registerGeneric() {
        this.registerTypeNameAndType();
        this.registerTypes(this.types);

        return BasicMapperTypedGenericRunnable.with(this);
    }

    /**
     * Eventually called by {@link BasicMapperTypedGenericRunnable#run()}
     */
    void remove() {
        TYPENAME_TO_FACTORY.remove(this.typeName());
        TYPENAME_TO_FACTORY.remove(this.type().getName());

        this.types.stream()
                .map(Class::getName)
                .forEach(TYPENAME_TO_FACTORY::remove);
    }
