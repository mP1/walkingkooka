 ...
 */
final class DomEntityMap extends DomMap<DomName, DomEntity> {

    static Map<DomName, DomEntity> from(final NamedNodeMap entities) {
        return isEmpty(entities) ?
                Maps.empty() :
                new DomEntityMap(entities);
    }

    private DomEntityMap(final NamedNodeMap entities) {
        super(entities);
    }
