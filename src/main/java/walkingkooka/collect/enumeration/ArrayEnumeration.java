...
/**
 * An {@link Enumeration} over an array.
 */
final class ArrayEnumeration<T> implements Enumeration<T> {

    static <T> ArrayEnumeration<T> with(final Object[] items) {
        Objects.requireNonNull(items, "items");

        return new ArrayEnumeration<T>(items.clone());
    }

    /**
     * Private constructor
     */
    private ArrayEnumeration(final Object[] items) {
        super();
        this.items = items;
        this.next = 0;
    }
