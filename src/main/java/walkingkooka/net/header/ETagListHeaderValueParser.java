...
final class ETagListHeaderValueParser extends ETagHeaderValueParser {

    static List<ETag> parseList(final String text) {
        final ETagListHeaderValueParser parser = new ETagListHeaderValueParser(text);
        parser.parse();
        return Lists.readOnly(parser.etags);
    }

    private ETagListHeaderValueParser(final String text) {
        super(text);
    }
