 ...
 */
abstract class IndentingPrinterTemplate implements IndentingPrinter {
    private final static char NL = '\n';

    private final static char CR = '\r';

    private final static char START_OF_NEW_LINE = IndentingPrinterTemplate.NL;

    /**
     * Package private constructor to limit sub classing.
     */
    IndentingPrinterTemplate(final Printer printer) {
        super();

        this.printer = printer;
        this.indentation = Indentation.EMPTY;
        this.previous = IndentingPrinterTemplate.START_OF_NEW_LINE;
    }
    ...
    private void print0(final CharSequence chars) {
        ...
        for (int i = 0; i < end; i++) {
            ...
            if ((IndentingPrinterTemplate.NL == previous) || (
                    (previous == IndentingPrinterTemplate.CR) && (IndentingPrinterTemplate.NL
                            != c))) {
                if (start != i) {
    ...
    @Override final public LineEnding lineEnding() throws PrinterException {
        return this.printer.lineEnding();
    }

    // IndentingPrinter

    @Override final public void indent(final Indentation indentation) throws PrinterException {
     ...
     */
    @Override final public void outdent() throws PrinterException {
        try {
     ...
     */
    @Override final public void lineStart() throws PrinterException {
        final char previous = this.previous;
        if ((IndentingPrinterTemplate.CR != previous) && (IndentingPrinterTemplate.NL != previous)) {
            final Printer printer = this.printer;
    ...
    @Override final public void flush() throws PrinterException {
        this.printer.flush();
    }

    @Override final public void close() throws PrinterException {
        this.printer.close();
    }
